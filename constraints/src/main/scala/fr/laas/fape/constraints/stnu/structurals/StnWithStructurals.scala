...
import fr.laas.fape.constraints.stnu.{
  Controllability,
  InconsistentTemporalNetwork,
  STNU
}
...
object StnWithStructurals {
  ...
  val INF: Int =
    Int.MaxValue / 2 - 1 // set to avoid overflow on addition of int values
  val NIL: Int = 0
...
final class StnWithStructurals(
    private var nonRigidIndexes: Array[Int], // map: tp.id => index
  ...
  /** If true, the STNU will check that the network is Pseudo Controllable when
    * invoking isConsistent
    */
  ...
  /** Callbacks to be invoked whenever the earliest start time of a timepoint
    * changes
    */
  val earliestExecutionUpdatesListener = mutable.ArrayBuffer[TPRef => Unit]()

  /** Record a callback to be invoked whenever a the earliest start time of a
    * timepoint is updated
    */
  def addEarliestExecutionUpdateListener(callback: TPRef => Unit) {
    earliestExecutionUpdatesListener += callback
  }
  ...
  def timepoints: IList[TPRef] = {
    val flexibles = nonRigidIndexes.indices.iterator
      .filter(nonRigidIndexes(_) != -1)
      .map(_tpRefs(_))
      .toList
  ...
  private def hasFlexIndex(tp: TPRef): Boolean =
    nonRigidIndexes.size > tp.id && nonRigidIndexes(tp.id) != -1
  private def addFlexIndex(tp: TPRef, idx: Int): Unit = {
    if (nonRigidIndexes.size <= tp.id) { // grow
      val prevSize = nonRigidIndexes.length
      nonRigidIndexes =
        util.Arrays.copyOf(nonRigidIndexes, math.max(prevSize * 2, tp.id + 1))
      var i = prevSize
      while (i < nonRigidIndexes.length) {
        nonRigidIndexes(i) =
          -1 // make sure we get an early failure when trying to access an inconsistent state
        i += 1
      }
    }
  ...
  def timepointIndexed(tp: TPRef): Boolean = timepointByIndex.contains(tp)
  def timepointFromIndex(index: Int): TPRef = timepointByIndex(index)
  def indexFromTimepoint(tp: TPRef): Int = timepointByIndex.indexOf(tp)

  private def isKnown(tp: TPRef) =
    hasFlexIndex(tp) || rigidRelations.isAnchored(tp)
    ...
    c match {
      ...
      case _ =>
        throw new RuntimeException(
          "Constraint: " + c + " is not properly supported"
        )
    }
  }
  ...
  def concurrent(tp1: TPRef, tp2: TPRef) =
  ...
  private def beforeOrConcurrent(first: TPRef, second: TPRef) =
    rigidAwareDist(second, first) <= NIL
  private def strictlyBefore(first: TPRef, second: TPRef) =
    rigidAwareDist(second, first) < NIL
  private def between(tp: TPRef, min: TPRef, max: TPRef) =
    beforeOrConcurrent(min, tp) && beforeOrConcurrent(tp, max)
  private def strictlyBetween(tp: TPRef, min: TPRef, max: TPRef) =
  ...
  override def distanceUpdated(a: Int, b: Int): Unit = {
    ...
    if (dist.getDistance(a, b) + dist.getDistance(b, a) < 0) {
      if (debugging)
        assert(
          !consistencyWithBellmanFord(),
          "Problem with the consistency of the STN"
        )
      consistent = false
    ...
    val timepointsWithUpdatedStart =
      if (earliestExecutionUpdatesListener.nonEmpty && start.nonEmpty) {
        ...
        if (b == stIndex) // b the start timepoint (or its anchor)
          timepointFromIndex(a) :: rigidRelations.getTimepointsAnchoredTo(
            timepointFromIndex(a)
          )
        else
          Nil
      } else
    ...
    for (
      listener <- earliestExecutionUpdatesListener;
      tp <- timepointsWithUpdatedStart
    )
      listener.apply(tp)
  }
  ...
  override def isConsistent(): Boolean = {
    ...
    (!shouldCheckPseudoControllability ||
      contingentLinks.forall(l =>
        isDelayPossible(l.src, l.dst, l.min.lb) && isConstraintPossible(
          l.src,
          l.dst,
          l.max.ub
        )
  ...
  override protected def isConstraintPossible(
      u: TPRef,
      v: TPRef,
      w: Int
  ): Boolean =
  ...
  /** Computes the max delay from a given timepoint to all others using
    * Bellman-Ford on the original edges. This is expensive (O(V*E)) but is
    * useful for providing a reference to compare to when debugging.
    */
  private def distancesFromWithBellmanFord(from: TPRef): Array[Int] = {
    ...
    for (i <- 0 until numIters) {
      for (e <- originalEdges) {
        d(e.to.id) =
          Math.min(d(e.to.id), DistanceMatrix.plus(d(e.from.id), e.value))
      }
    }
    d
  }

  /** Computes the max delay between two timepoints using Bellman-Ford on the
    * original edges. This is expensive (O(V*E)) but is useful for providing a
    * reference to compare to when debugging.
    */
  private def distanceWithBellmanFord(from: TPRef, to: TPRef): Int = {
    distancesFromWithBellmanFord(from)(to.id)
  }

  /** Determine whether the STN is consistent using Bellman-Ford on the original
    * edges. This is expensive (O(V*E)) but is useful for providing a reference
    * to compare to when debugging.
    */
  ...
  override def enforceContingent(
      u: TPRef,
      v: TPRef,
      min: Int,
      max: Int
    ...
    addMinDelay(u, v, min)
    addMaxDelay(u, v, max)
    contingentLinks.append(
      new ContingentConstraint(
        u,
        v,
        IntExpression.lit(min),
        IntExpression.lit(max)
      )
    )
  }
  ...
  /** If there is a contingent constraint [min, max] between those two
    * timepoints, it returns Some((min, max). Otherwise, None is returned.
    */
  override def contingentDelay(
      from: TPRef,
      to: TPRef
  ): Option[(Integer, Integer)] =
    contingentLinks.find(l => l.src == from && l.dst == to) match {
      case Some(x) =>
        Some(x.min.lb.asInstanceOf[Integer], x.max.ub.asInstanceOf[Integer])
      case None => None
    }
  ...
  override def getConstraintsWithoutStructurals: IList[TemporalConstraint] = {
    ...
    def structuralNeighborhood(
        neighborhood: Set[TPRef],
        nextNeighbors: Set[TPRef]
    ): Set[TPRef] = {
      ...
      if (
        rigidRelations
          .isAnchored(tp) && !rigidRelations.anchorOf(tp).genre.isStructural
      ) {
    ...
    def connections(tp: TPRef) = {
      ...
      val nonStructuralNeighbours = originalEdges
        .filter(e =>
          structuralNeighbors.contains(e.from) || structuralNeighbors.contains(
            e.to
          )
        )
        .flatMap(e => e.from :: e.to :: Nil)
        ...
        structuralNeighbors
          .filter(x =>
            rigidRelations
              .isAnchored(x) && !rigidRelations.anchorOf(x).genre.isStructural
          )
    ...
    for (
      tp <- timepoints.asScala
      if tp.genre.isStructural && rigidRelations.isAnchor(tp)
    ) {
    ...
    for (
      tp <- timepoints.asScala if !tp.genre.isStructural && rigidRelations
        .isAnchored(tp) && !rigidRelations.anchorOf(tp).genre.isStructural
    ) {
    ...
    return new IList(
      contingentLinks.toList ++
        pairs.map(p =>
          new MinDelayConstraint(
            p._2,
            p._1,
            IntExpression.lit(minDelay(p._2, p._1))
          )
        )
    )
  }
  ...
  def getOriginalConstraints: IList[TemporalConstraint] = {
    new IList(
      originalEdges
        .map(e =>
          new MinDelayConstraint(e.to, e.from, IntExpression.lit(-e.value))
        )
